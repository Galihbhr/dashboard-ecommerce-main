...
def create_customer_demo_df(df):
    customers_demo_df = df.groupby(by="customer_state").order_id.nunique(). reset_index()
    customers_demo_df.rename(columns = {
...
with st.sidebar:
    ...
    start_date, end_date = st.date_input(
        label='Rentang Waktu',
        min_value=min_date,  # Tanggal minimum
        max_value=max_date,  # Tanggal maksimum
        value=[min_date, max_date]  # Rentang default
    )
...
bystate_df = create_customer_demo_df(main_df) 
